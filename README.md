...
### Data Model
According to the [exploratory data analysis](eda.ipynb), we found that we can access deeper and more detailed information by connecting the Iowa Liquor Sales dataset with other datasets. We want to construct a data warehouse using the liquor sales data with three other datasets connected to it.
Each of these datasets has a different connection to the Iowa Liquor Sales dataset. For example, the liquor data and the census data have a `County` column in common, so we can join these datasets together using the `County` column. Hence, in order to create one database that can add several different perspectives to the Iowa Liquor Sales dataset, we should use a **star-schema** with the liquor dataset as the fact table and the other datasets as dimension tables.

### Tools and Technologies
In order to achieve the goal above, we will utilize the following tools and technologies:

- **Amazon Redshift**
	is a data warehouse service on cloud, which is a high performant and highly scalable. With Amazon Redshift, we can make our data easily accessible for analysis using BI tools.
...
- **Amazon S3**
	...
	store the data files, which could be 1TB in size, in a local computer. Also, the compatibility between S3 and Redshift makes the ETL process easy.   

- **Apache Airflow**
	is a workflow management platform. Since our ETL pipeline will load data from four different sources and insert six tables, we have many tasks to manage. We need to know which task is functioning well and which is not. We also need to pararellize some of the tasks. Airflow can take care of those issues well.  

- **Apache Spark**
	is a distributed cluster-computing framework. Spark makes it possible to analyze and manipulate data that is too big to process on a local computer. We will use Apache Spark for an exploratory data analysis.  
